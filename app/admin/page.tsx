...
export default function AdminDashboard() {
  ...
  const handleMessageUpdate = (event: any) => {
    if (activeCustomer && event.customerId === activeCustomer.id) {
      setMessages((prev) => prev.map((msg) => (msg.id === event.data.id ? event.data : msg)))
    }
  }
  ...
  const handleCustomerStatus = (event: any) => {
    setCustomers((prev) =>
      prev.map((customer) =>
        customer.id === event.customerId ? { ...customer, status: event.data.status } : customer,
      ),
    )
  }
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (activeCustomer) {
      chatChannel.sendTypingIndicator(activeCustomer.id, e.target.value.length > 0, true)
    ...
    <div className="flex h-screen flex-col">
      ...
      <header className="flex items-center justify-between p-4 apple-header">
        <div className="flex items-center space-x-3">
          <AutoPartsLogo />
          <h1 className="text-xl font-semibold text-gray-900">CC Auto Parts Admin Dashboard</h1>
        </div>
        <div className="flex items-center space-x-4">
          {customers.length > 1 && (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="flex items-center gap-2 bg-gray-100 border-gray-200 text-gray-800">
              ...
              <DropdownMenuContent align="end" className="bg-white border border-gray-200">
                {customers.map((customer) => (
                  <DropdownMenuItem key={customer.id} onClick={() => handleCustomerSelect(customer)}>
                    <div className="flex items-center gap-2">
                      <div
                        className={`h-2 w-2 rounded-full ${customer.status === "online" ? "bg-green-500" : "bg-gray-300"}`}
                      />
                      {customer.name}
                    </div>
                  </DropdownMenuItem>
      ...
      <div className="flex flex-1 overflow-hidden p-6 gap-6 bg-[#f5f5f7]">
        ...
        <div className="flex flex-1 flex-col apple-card overflow-hidden">
          {activeCustomer ? (
            <>
              <div className="p-4 border-b border-gray-100 bg-white">
                <div className="flex items-center space-x-2">
                  <Avatar className="h-10 w-10">
                    <AvatarFallback className="bg-blue-600">{activeCustomer.name[0]}</AvatarFallback>
                  </Avatar>
                  <div>
                    <h2 className="text-lg font-medium text-gray-900">{activeCustomer.name}</h2>
                    <p className="text-sm text-gray-500">{activeCustomer.status === "online" ? "Online" : "Offline"}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6">
                {messages.length === 0 ? (
                  <div className="flex h-full items-center justify-center text-gray-400">
                    <p>No messages yet. Wait for the customer to start the conversation.</p>
                  </div>
                ) : (
                  messages.map((message) => (
                    ...
                    >
                      <div
                        className={`flex max-w-[80%] ${message.sender === "agent" ? "flex-row-reverse" : "flex-row"}`}
                      >
                        <Avatar className={`${message.sender === "agent" ? "ml-2" : "mr-2"} h-8 w-8`}>
                          <AvatarFallback className={message.sender === "agent" ? "bg-blue-600" : "bg-gray-400"}>
                            {message.sender === "agent" ? "A" : activeCustomer.name[0]}
                          </AvatarFallback>
                        </Avatar>
                        <div
                          className={`p-3 ${
                            message.sender === "agent" ? "apple-message-bubble-customer" : "apple-message-bubble-agent"
                          }`}
                        >
                          {message.content}
                          <div
              ...
              <div className="p-4 border-t border-gray-100 bg-white">
                <form onSubmit={handleSendMessage} className="flex space-x-2">
                  ...
                  />
                  <Button type="submit" disabled={!inputMessage.trim()} className="apple-button">
            ...
            <div className="flex h-full flex-col items-center justify-center p-6 text-gray-500">
              ...
              <h3 className="text-xl font-medium mb-2 text-gray-900">No Active Chats</h3>
              <p className="text-center max-w-md">
                There are currently no customers chatting. When customers start a conversation, they will appear here.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="w-1/4 apple-card p-6">
          {activeCustomer ? (
            <>
              <h2 className="text-lg font-medium mb-4 text-gray-900">Customer Details</h2>
              <Card className="border-gray-200 bg-white shadow-sm">
                <CardHeader className="pb-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <CardTitle className="text-gray-900">{activeCustomer.name}</CardTitle>
                  </div>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <div className="space-y-1">
                      <p className="text-sm text-gray-700">
                        <span className="font-medium">Email:</span> {activeCustomer.email}
                      </p>
                      <p className="text-sm text-gray-700">
                        <span className="font-medium">Phone:</span> {activeCustomer.phone}
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <Separator className="my-2 bg-gray-200" />
                    <p className="text-sm text-gray-700">{activeCustomer.vehicleInfo}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <Separator className="my-2 bg-gray-200" />
                    <p className="text-sm text-gray-700">
                      <span className="font-medium">Last Purchase:</span> {activeCustomer.lastPurchase}
                    </p>
                  </div>
                </CardContent>
              </Card>
            ...
            <div className="flex h-full flex-col items-center justify-center text-gray-500">
              <h3 className="text-lg font-medium mb-2 text-gray-900">No Customer Selected</h3>
              <p className="text-center">Customer details will appear here when a chat is active.</p>
            </div>
          )}
        </div>
      </div>
